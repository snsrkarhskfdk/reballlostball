...
- 2026-06-04 사용자가 관리자 페이지 데이터 초기화를 예약 명령으로 남겼다. 지금 바로 실행하지 않는다.
- 실행 조건: 같은 `리볼로스트볼` 프로젝트의 모든 기능/디자인/Figma/QA/배포 task가 완료되면 사용자에게 다시 묻지 않고 자동 실행한다.
- 예약 작업:
  - 관리자 페이지 안의 가짜 운영 데이터를 초기화한다.
  - 현재 실제 상태 기준으로 회원은 `0명`, 주문은 `0건`이므로 관리자 요약/테이블/도넛/빠른 처리에서 더미 회원·주문 숫자를 제거한다.
  - 결제완료, 배송준비중, 취소/반품요청, 문의, 리뷰, 매출, 정산, 포스기 현장 주문 등 실제 데이터가 없는 항목은 `0` 또는 빈 목록으로 맞춘다.
  - 단, 전체 상품수, 쿠폰 종류수, 설정/권한 수치는 유지한다.
  - 전체 상품수와 쿠폰 종류수는 사용자가 실제로 올려놓은 상품/쿠폰 데이터 수량 기준으로 맞춘다.
  - 완료 후 검증: 관리자 각 탭의 요약 숫자와 테이블 행 수 불일치 0, 엑셀 다운로드의 행 수 불일치 0, 빈 상태 UI 누락 0.
