...
function adminStatsForTab(tab) {
  ...
  const stats = {
    ...
    customer: [
      ["회원", `${customerRows.length}명`, "활성 고객", "user"],
      ["신규 고객", `${customerRows.filter((row) => row.subValue.includes("신규")).length}명`, "이번 주", "star"],
...
function adminChartValues(tab) {
  ...
  const values = {
    ...
    settlement: [0, 0, 0],
    customer: [customerRows.length, customerRows.filter((row) => row.subValue.includes("신규")).length, adminInquiryPosts().filter((post) => !post.answer?.trim()).length],
...
function adminChartCenter(tab) {
  ...
  if (tab === "customer") return `${adminCustomerRows().length}명`;
