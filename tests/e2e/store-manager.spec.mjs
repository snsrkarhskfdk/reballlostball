...
function query(table){
  const q={
    select(){return q}, eq(){return Promise.resolve({data:table==="user_roles"?[{role}]:[],error:null})}, order(){return Promise.resolve({data:table==="products"?productRows:[],error:null})}
  };
  return q;
}
...
async function installMocks(page, role = "owner") {
  ...
  await page.route("https://fake.supabase.test/functions/v1/**", async (route) => {
    ...
    if (url.pathname.endsWith("/admin-console") && route.request().method() === "GET") {
      ...
      else if (view === "orders") body = { orders:[{id:"cccccccc-cccc-4ccc-8ccc-cccccccccccc",order_no:"RB-TEST",status:"paid",payment_status:"done",payment_method:"card",total_krw:26000,refund_amount:0,created_at:new Date().toISOString(),address_snapshot:{receiverName:"테스터",receiverPhone:"01012345678",roadAddress:"부천시 소사구 경인로10번길 34"},order_items:[{product_name:"테스트 로스트볼",variant_name:"A 10구",qty:1,line_total_krw:22500}],payment:{method:"card",status:"done",approved_amount:26000,canceled_amount:0},notes:[],canCancel:${'role === "store" ? "false" : "true"'}}] };
...
test("owner admin sees the full operations console", async ({ page }) => {
...
test("store manager gets daily operations without owner or refund controls", async ({ page }) => {
  await installMocks(page, "store");
  for (const tab of ["dashboard","orders","products","shipping"]) await expect(page.locator(`[data-tab="${tab}"]`)).toBeVisible();
  for (const tab of ["members","audit","settings","staff"]) await expect(page.locator(`[data-tab="${tab}"]`)).toBeHidden();
  await page.locator('[data-tab="orders"]').click();
  await expect(page.locator("[data-all-order-list]")).toContainText("RB-TEST");
  await expect(page.locator("[data-cancel-order]")).toHaveCount(0);
});

test("operations console stays inside a 390px mobile viewport", async ({ page }) => {
