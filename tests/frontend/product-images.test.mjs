...
test("variant image resolution matches model aliases and UI grades", () => {
  ...
  assert.equal(
    resolveProductVariantImage(product, { model: "투어 X", grade: "A+", color: "화이트" }),
...
test("an explicit Supabase thumbnail is preserved and blank thumbnails use the supplied mapping", () => {
  ...
  const selection = { model: "투어 X", grade: "A+", color: "화이트" };
  ...
  assert.equal(
    resolveRemoteVariantImage(product, selection, "ball-bridgestone.png", "fallback.webp"),
    "product-variants/bridgestone-tour-b-s.webp"
  );
});
...
test("each supplied brand exposes its representative photo on the homepage card", () => {
  ...
  assert.deepEqual(products.find((item) => item.brandSlug === "volvik").models, ["볼빅"]);
  assert.ok(products.find((item) => item.brandSlug === "callaway").models.includes("크롬·ERC·트리플트랙"));
});
